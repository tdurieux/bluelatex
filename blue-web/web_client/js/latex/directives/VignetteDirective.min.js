angular.module('bluelatex.Latex.Directives.Vignette', ['bluelatex.Paper.Services.Paper'])
  .directive('blVignette', ['PaperService', function(PaperService) {
    return {
      ...
      'template': '<img src="{{getUrlVignette()}}&{{revision}}" ng-if="type==\'image\'"><div class="container"><canvas ng-if="type==\'pdf\'"></canvas><div class="textLayer" ng-if="type==\'pdf\'"></div><div class="hightlights" ng-if="synctex"><div class="hightlight_line" ng-repeat="hightlight in hightlights" style="height:{{hightlight.height}};width:{{hightlight.width}};left:{{hightlight.left}};top:{{hightlight.top}}"></div></div></div>'
    };
  }]);